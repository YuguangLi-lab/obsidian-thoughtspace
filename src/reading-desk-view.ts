...
export class ReadingDesk extends Modal{
 private options:ReadingOptions={query:'',status:'all',sort:'board',onlySelected:false};private activeId?:string;private list!:HTMLElement;private page!:HTMLElement;private progress!:HTMLElement;private counter!:HTMLElement;private previewScope?:Component;private lifecycle?:Component;private generation=0;private pageKey='';private alive=false;private statusSelect!:HTMLSelectElement;private filterCaption?:HTMLElement;private companion!:HTMLElement;private asideTab:'queue'|'outline'|'relations'='queue';private asideTabs!:HTMLElement;private headings:HTMLElement[]=[];
 constructor(app:App,private host:ReadingHost){super(app);this.options.onlySelected=host.ids.size>0;}
 private run(fn:()=>unknown){try{Promise.resolve(fn()).catch(e=>new Notice(String(e)));}catch(e){new Notice(String(e));}}
 ...
 const heading=this.contentEl.createDiv('ts-reading-heading');heading.createDiv({cls:'ts-reading-context',text:this.host.title});this.progress=heading.createDiv('ts-reading-progress');
 const tools=this.contentEl.createDiv('ts-reading-tools'),search=tools.createEl('input',{type:'search',attr:{placeholder:'搜索标题、路径或文本…','aria-label':'搜索阅读内容'}});search.oninput=()=>this.run(()=>{this.options.query=search.value;this.renderList();});
 ...
 const body=this.contentEl.createDiv('ts-reading-layout'),aside=body.createDiv('ts-reading-aside');this.asideTabs=aside.createDiv({cls:'ts-reading-aside-tabs',attr:{role:'group','aria-label':'阅读导航'}});
 ...
 this.companion=aside.createDiv('ts-reading-companion');this.counter=aside.createDiv('ts-reading-count');this.list=aside.createDiv({cls:'ts-reading-list',attr:{role:'group','aria-label':'阅读列表'}});this.page=body.createDiv('ts-reading-page');
 const footer=this.contentEl.createDiv('ts-reading-footer');this.button(footer,'复制回顾清单','clipboard-list',async()=>{await navigator.clipboard.writeText(readingDigest(this.items(),this.host.title));new Notice('已复制当前筛选范围的回顾清单');});const focus=this.button(footer,'专注正文','panel-left-close',()=>{const active=this.modalEl.classList.toggle('ts-reading-focused');focus.setAttribute('aria-pressed',String(active));});focus.setAttribute('aria-pressed','false');
 const font=footer.createEl('select',{attr:{'aria-label':'本次阅读字号'}});for(const size of [14,16,18,20])font.createEl('option',{value:String(size),text:`${size} px`});font.value=String(this.host.settings.readingSize);font.onchange=()=>this.modalEl.style.setProperty('--ts-reading-size',`${font.value}px`);
 this.lifecycle=new Component();this.lifecycle.load();this.lifecycle.registerDomEvent(this.modalEl,'keydown',e=>{
  if(!e.altKey||e.ctrlKey||e.metaKey||e.shiftKey||!['ArrowLeft','ArrowRight'].includes(e.key)||(e.target as HTMLElement).closest('input,textarea,select,[contenteditable=true]'))return;
 ...
 if(!items.length){this.headings=[];this.renderCompanion();this.list.createDiv({cls:'ts-reading-empty',text:'没有匹配内容，试试更换状态或范围。'});this.page.empty();this.pageKey='';this.generation++;this.previewScope?.unload();this.previewScope=undefined;return;}
 ...
 const actions=head.createDiv('ts-reading-actions');for(const [status,label]of Object.entries(reviewLabels)){const b=this.button(actions,label,status==='done'?'check-circle-2':status==='reading'?'book-open':'clock',()=>{this.host.commit(board=>markReading(board,new Set([n.id]),status as ReviewState));this.renderList();});b.dataset.readingStatus=status;b.disabled=!!n.locked;b.setAttribute('aria-pressed',String((n.review||'later')===status));}
 const finish=this.button(actions,'已读并下一篇','list-checks',()=>{const items=this.items(),index=items.findIndex(item=>item.id===n.id),next=items[index+1]?.id;this.host.commit(board=>markReading(board,new Set([n.id]),'done'));this.activeId=next||n.id;this.renderList();});finish.dataset.readingFinish='true';finish.disabled=!!n.locked;
 this.button(actions,'定位白板','locate-fixed',()=>{this.host.reveal(n.id);this.close();});if(file instanceof TFile)this.button(actions,'右侧打开原文','panel-right',async()=>{await this.host.open(file);this.close();});
 const content=this.page.createDiv('ts-reading-prose'),nav=this.page.createDiv('ts-reading-pagination');
 const previous=this.button(nav,'上一篇','arrow-left',()=>this.step(-1));previous.dataset.readingPrevious='true';
 nav.createSpan({attr:{'data-reading-position':'true'}});
 const next=this.button(nav,'下一篇','arrow-right',()=>this.step(1));next.dataset.readingNext='true';this.syncPageControls(n);
 ...
 private syncPageControls(n:Card){
  this.page.querySelectorAll<HTMLButtonElement>('[data-reading-status]').forEach(button=>{button.disabled=!!n.locked;button.setAttribute('aria-pressed',String(button.dataset.readingStatus===(n.review||'later')));});
  const finish=this.page.querySelector<HTMLButtonElement>('[data-reading-finish]');if(finish)finish.disabled=!!n.locked;
  const items=this.items(),index=items.findIndex(item=>item.id===n.id);
  const previous=this.page.querySelector<HTMLButtonElement>('[data-reading-previous]'),next=this.page.querySelector<HTMLButtonElement>('[data-reading-next]'),position=this.page.querySelector<HTMLElement>('[data-reading-position]');
 ...
 private renderCompanion(){
  ...
  for(const relation of relations.slice(0,100)){
   const button=this.button(this.companion,readingTitle(relation.node),'arrow-up-right',()=>{
    ...
    this.activeId=relation.node.id;this.renderList();
