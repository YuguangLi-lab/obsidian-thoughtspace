...
export function renderPresetGallery(host:HTMLElement,action:string,apply:(board:Board)=>Promise<void>|void){
 ...
 const draw=()=>{try{
  current=presetMindmap(selected,title.value,undefined,n=>fitTextNode(n,host));current=mindmapPlan(current,current.nodes[0].id,{layout:layout.value as MindmapLayout,density:'standard',depth:'all',rainbow:true}).board;
  preview.empty();const svg=new DOMParser().parseFromString(topicSvg(current,current.nodes[0].id),'image/svg+xml').documentElement;const scene=host.ownerDocument.importNode(svg,true) as unknown as SVGSVGElement;
  // The in-app preview follows the theme; the standalone export keeps its paper palette.
  scene.querySelectorAll('rect[fill="#fff"]').forEach(rect=>rect.setAttribute('fill','var(--background-primary)'));
  scene.querySelectorAll('text').forEach(el=>el.remove());for(const n of current.nodes)createTopicPreviewLabel(scene,n);preview.append(scene);
  status.setText(`${current.nodes.length} 个主题 · ${mindmapLayouts[layout.value as MindmapLayout]} · 自动尺寸与排版`);confirm.disabled=false;
 }catch(e){current=undefined;status.setText(String(e));confirm.disabled=true;}};
