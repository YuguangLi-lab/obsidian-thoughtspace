...
export function resized(original:Card,dx:number,dy:number,ratio:boolean){const minWidth=original.kind==='text'?80:180;let width=Math.max(minWidth,original.width+dx),height=Math.max(100,original.height+dy);if(ratio){const scale=Math.max(minWidth/original.width,100/original.height,Math.abs(dx/original.width)>=Math.abs(dy/original.height)?width/original.width:height/original.height);width=original.width*scale;height=original.height*scale;}return{width,height};}
