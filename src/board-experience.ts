...
export type NodeStyle=Pick<Card,'transparent'|'fillColor'|'color'|'textColor'|'fontFamily'|'fontSize'|'textAlign'|'customBorder'|'borderStyle'|'borderWidth'|'sectionDivider'>;
const styleKeys=['transparent','fillColor','color','textColor','fontFamily','fontSize','textAlign','customBorder','borderStyle','borderWidth','sectionDivider'] as const;
export function readNodeStyle(n:Card):NodeStyle{const result={color:n.color} as NodeStyle;for(const k of styleKeys)if(n[k]!==undefined&&(k!=='sectionDivider'||n.kind==='section'))Object.assign(result,{[k]:n[k]});return result;}
export function applyNodeStyle(board:Board,ids:ReadonlySet<string>,style:NodeStyle){for(const n of board.nodes.filter(n=>ids.has(n.id)&&!n.locked))for(const k of styleKeys){if((k==='fillColor'||k==='transparent')&&n.kind!=='card'&&n.kind!=='text'&&n.kind!=='section')continue;if(k==='sectionDivider'&&n.kind!=='section')continue;if(style[k]===undefined)delete n[k];else Object.assign(n,{[k]:style[k]});}}
