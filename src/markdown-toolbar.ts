...
export function markdownToolbar(host:HTMLElement,editor:MarkdownToolbarEditor){
 ...
 const insert=group('插入内容');const noteLink=editor.linkNote&&editor.canLinkNote!==false?action(insert,'选中文字创建或关联笔记','file-symlink',()=>{try{editor.linkNote?.();}catch(e){new Notice(String(e));}}):undefined;command(insert,'link','链接 · ⌘/Ctrl+K','link');command(insert,'table','插入表格','table-2');command(insert,'image','图片链接','image');select(insert,'插入',[['wikilink','双链 [[笔记]]'],['image','图片链接'],['codeblock','代码块'],['table','表格'],['callout','提示块'],['rule','分隔线'],['sup','上标'],['sub','下标'],['math','行内公式'],['mathblock','独立公式块'],['comment','隐藏注释']],'circle-plus');
