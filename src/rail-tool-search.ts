...
export function installRailToolSearch(panel:HTMLElement,tools:HTMLElement){
 ...
 const groups=Array.from(tools.querySelectorAll<HTMLElement>('.ts-tool-cluster')).map(element=>({element,hidden:element.hidden}));
 const entries=Array.from(tools.querySelectorAll<HTMLButtonElement>('button')).map(element=>({element,hidden:element.hidden,group:element.closest<HTMLElement>('.ts-tool-cluster'),label:normalize([element.getAttribute('aria-label'),element.title,element.closest('.ts-tool-cluster')?.getAttribute('aria-label')].filter(Boolean).join(' '))}));
 let disposed=false,composing=false;
 const refresh=()=>{
  if(disposed)return;const tokens=normalize(input.value).split(/\s+/u).filter(Boolean);let count=0;
  for(const entry of entries){const hidden=entry.hidden||groups.some(group=>group.element===entry.group&&group.hidden)||!tokens.every(token=>entry.label.includes(token));if(entry.element.hidden!==hidden)entry.element.hidden=hidden;if(!hidden)count++;}
  for(const group of groups){const hidden=group.hidden||!entries.some(entry=>entry.group===group.element&&!entry.element.hidden);if(group.element.hidden!==hidden)group.element.hidden=hidden;}
 ...
 const visible=()=>entries.map(entry=>entry.element).filter(element=>!element.disabled&&!element.hidden&&element.getClientRects().length>0);
 ...
 input.addEventListener('input',onInput);input.addEventListener('compositionstart',onStart);input.addEventListener('compositionend',onEnd);clear.addEventListener('click',reset);panel.addEventListener('keydown',keys,true);refresh();
 return{open:reset,dispose:()=>{if(disposed)return;disposed=true;input.removeEventListener('input',onInput);input.removeEventListener('compositionstart',onStart);input.removeEventListener('compositionend',onEnd);clear.removeEventListener('click',reset);panel.removeEventListener('keydown',keys,true);header.remove();empty.remove();for(const entry of entries)entry.element.hidden=entry.hidden;for(const group of groups)group.element.hidden=group.hidden;}};
}
