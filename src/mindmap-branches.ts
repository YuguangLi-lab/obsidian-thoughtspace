...
import {branchState,mindmapRoot,layoutMindmap,unfoldAncestors} from './mindmap';
...
export function arrangeBranchDepth(source:Board,selected:string,depth:BranchDepth):TopicResult{
 if(!['focus','all','1','2'].includes(depth))throw Error('展开方式无效');const b=clone(source),root=mindmapRoot(b,selected),rows=topicRows(b,root);if(rows.some(r=>r.node.locked))throw Error('主题树中有锁定对象，请先解锁');
 const branch=topicRows(b,selected),ids=new Set(branch.map(r=>r.node.id)),{children,parents}=branchState(b);
...
export function saveTopicRelation(source:Board,selected:string,value:TopicRelation,makeId:()=>string=uid):TopicResult{
 const b=clone(source),root=mindmapRoot(b,selected),tree=new Set(topicRows(b,root).map(r=>r.node.id)),nodes=new Map(b.nodes.map(n=>[n.id,n]));
