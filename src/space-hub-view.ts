...
export class SpaceHubModal extends Modal {
  ...
  onOpen(){
    ...
    const head=this.contentEl.createDiv('ts-hub-head');head.createEl('p',{text:'从材料到思考，找到下一步。'});const actions=head.createDiv('ts-hub-quick');this.action(actions,'收集笔记','plus',()=>{this.close();this.host.capture();},'is-primary');this.action(actions,'新建白板','panels-top-left',()=>{this.close();this.host.createBoard();});this.action(actions,'日历','calendar-days',()=>{this.close();return this.host.calendar();});
    this.metrics=this.contentEl.createDiv('ts-hub-metrics');const body=this.contentEl.createDiv('ts-hub-body');this.nav=body.createEl('nav',{cls:'ts-hub-nav',attr:{'aria-label':'空间总览导航'}});
    const workspace=body.createDiv('ts-hub-workspace'),tools=workspace.createDiv('ts-hub-tools');this.search=tools.createEl('input',{type:'search',attr:{placeholder:'搜索标题、路径、#标签…','aria-label':'搜索空间总览',maxlength:'200'}});this.search.oninput=()=>{this.filter.query=this.search.value;this.page=0;this.renderResults();};
    const filters=tools.createDiv('ts-hub-filters');this.tag=filters.createEl('select',{attr:{'aria-label':'按标签筛选'}});this.tag.onchange=()=>{this.filter.tag=this.tag.value;this.page=0;this.renderResults();};this.sort=filters.createEl('select',{attr:{'aria-label':'总览排序'}});for(const[v,t]of [['updated','最近修改'],['title','名称排序'],['size','内容 / 使用次数']])this.sort.createEl('option',{value:v,text:t});this.sort.onchange=()=>{this.filter.sort=this.sort.value as HubFilter['sort'];this.page=0;this.renderResults();};
    const label=filters.createEl('label',{cls:'ts-hub-journal-toggle'});this.journals=label.createEl('input',{type:'checkbox'});label.createSpan({text:'含日记'});this.journals.onchange=()=>{this.filter.includeJournals=this.journals.checked;this.page=0;this.renderResults();};
    this.action(filters,'切换列表 / 画廊','layout-grid',async()=>{await this.host.save({...this.host.preferences(),view:this.host.preferences().view==='gallery'?'list':'gallery'});if(this.active)this.renderResults();});
    this.refreshButton=this.action(filters,'刷新索引','refresh-cw',()=>this.load());this.state=workspace.createDiv({cls:'ts-hub-state',attr:{role:'status','aria-live':'polite'}});
    this.main=workspace.createDiv('ts-hub-results');this.detail=body.createEl('aside',{cls:'ts-hub-detail',attr:{'aria-label':'内容详情'}});this.footer=this.contentEl.createDiv('ts-hub-footer');
  ...
  async load(){
    ...
    try{
      ...
      this.state.setText(errors.length?`${errors.length} 张白板无法索引；收件箱暂不判定未归属。点击右侧“索引问题”查看。`:this.changes!==changes?'资料在索引期间发生变化，请刷新后核对。':'索引已更新 · 搜索标题、路径和原生标签');this.renderMetrics();this.renderNav();this.renderResults();this.renderDetail();
  ...
  private renderResults(){
    ...
    for(const item of this.results.slice(this.page*48,(this.page+1)*48)){
      const board='objects'in item,row=this.main.createDiv({cls:`ts-hub-result ${board?'is-board':'is-note'}`});row.toggleClass('is-focused',this.focused===item.path);
  ...
  private focus(path:string){this.focused=path;this.renderResults();this.renderDetail();const heading=this.detail.querySelector<HTMLElement>('h3');if(heading){heading.tabIndex=-1;heading.focus({preventScroll:true});}if(this.modalEl.clientWidth<840)this.detail.scrollIntoView({block:'nearest'});}
  private renderDetail(){
    this.detail.empty();const item=this.index.boards.find(b=>b.path===this.focused)||this.index.notes.find(n=>n.path===this.focused);
    if(!item){this.detail.createDiv({cls:'ts-hub-detail-mark',text:'整理，从这里开始'});this.detail.createEl('h3',{text:'给材料找到位置'});this.detail.createEl('p',{text:'选择一张白板查看内容概况，或选择笔记查看它在哪些白板中使用。'});if(this.host.target)this.detail.createDiv({cls:'ts-hub-target',text:'当前白板\n'+this.host.target.title});}
    else{const board='objects'in item;this.detail.createDiv({cls:'ts-hub-detail-mark',text:board?'白板详情':'笔记详情'});this.detail.createEl('h3',{text:item.title});this.detail.createDiv({cls:'ts-hub-detail-path',text:item.path});this.action(this.detail,board?'打开白板':'右侧打开笔记',board?'arrow-up-right':'panel-right',async()=>{const f=this.file(item.path);this.close();if(board)await this.host.openBoard(f);else await this.host.openNote(f);},'is-primary');
  ...
  private renderFooter(){this.footer.empty();this.footer.createSpan({text:`${this.results.length} 项结果 · 第 ${this.page+1} / ${Math.max(1,Math.ceil(this.results.length/48))} 页`});this.action(this.footer,'上一页','chevron-left',()=>{this.page--;this.renderResults();}).disabled=this.page===0;this.action(this.footer,'下一页','chevron-right',()=>{this.page++;this.renderResults();}).disabled=(this.page+1)*48>=this.results.length;
