...
export class LayoutRefreshQueue {
 ...
 constructor(private refresh:()=>void,private clock:RefreshClock={frame:run=>requestAnimationFrame(run),cancelFrame:id=>cancelAnimationFrame(id),later:(run,ms)=>window.setTimeout(run,ms),cancelLater:id=>window.clearTimeout(id)}){}
