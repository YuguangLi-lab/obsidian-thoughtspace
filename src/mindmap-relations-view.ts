...
import {topicTreeIndex,topicLabel,TopicResult} from './mindmap-editor';
...
export function renderTopicRelations(host:HTMLElement,board:Board,selected:string,apply:(r:TopicResult)=>void){
 const index=topicTreeIndex(board),rows=index.rows(index.rootOf(selected)),byId=index.nodes,source=byId.get(selected)!,rowIds=new Set(rows.map(r=>r.node.id));
 ...
 const fill=(chosen=target.value)=>{
  target.empty();const q=search.value.trim().toLocaleLowerCase(),visible:typeof rows=[];let matches=0,picked:typeof rows[number]|undefined;
  for(const row of rows){if(row.node.id===selected)continue;if(!picked&&row.node.id===chosen)picked=row;if(q&&!topicLabel(row.node).toLocaleLowerCase().includes(q))continue;matches++;if(visible.length<80)visible.push(row);}
  if(picked&&!visible.some(r=>r.node.id===chosen)){if(visible.length===80)visible.pop();visible.unshift(picked);}
  for(const {node}of visible)target.createEl('option',{value:node.id,text:topicLabel(node).replace(/\s+/g,' ').slice(0,100)});
  target.value=visible.some(r=>r.node.id===chosen)?chosen:visible[0]?.node.id||'';hint.setText(matches>80?`${matches} 项匹配 · 显示前 80 项，请继续搜索`:`${matches} 个可选主题`);save.disabled=!target.value||!!source.locked;
 };
 ...
 for(const edge of relations.slice(0,80)){const outgoing=edge.from===selected,other=byId.get(outgoing?edge.to:edge.from),row=list.createDiv('ts-mm-relation-row');row.createEl('strong',{text:`${outgoing?'→':'←'} ${other?topicLabel(other).slice(0,60):'主题已移除'}`});row.createSpan({text:edge.label||'未命名关系'});const buttons=row.createDiv();if(outgoing&&rowIds.has(edge.to)){const edit=buttons.createEl('button',{text:'编辑'});edit.onclick=()=>{editing=edge.id;search.value='';fill(edge.to);label.value=edge.label;direction.value=edge.direction||'forward';save.setText('保存关联线');};}const remove=buttons.createEl('button',{text:'移除'});remove.onclick=()=>{try{apply(removeTopicRelation(board,selected,edge.id));}catch(e){status.setText(String(e));}};}
