...
export class BoardSearchModal extends Modal{
 ...
 onOpen(){this.closed=false;themeSurface(this.modalEl);this.modalEl.addClass('ts-board-search');this.titleEl.setText('搜索白板');this.titleEl.createEl('small',{text:this.host.title});
  const bar=this.contentEl.createDiv('ts-board-search-bar');setIcon(bar.createSpan(),'search');this.input=bar.createEl('input',{type:'search',attr:{placeholder:'标题、文本、#标签、分组或文件路径…','aria-label':'搜索白板内容'}});
  ...
  const options=this.contentEl.createDiv('ts-board-search-options'),label=options.createEl('label'),full=label.createEl('input',{type:'checkbox'});label.createSpan({text:'包含笔记正文'});full.checked=this.full;full.onchange=()=>{this.full=full.checked;this.run(()=>this.refresh());};this.status=options.createSpan({attr:{role:'status','aria-live':'polite'}});
  ...
  const footer=this.contentEl.createDiv('ts-board-search-footer');footer.createSpan({text:'↑ ↓ 选择 · Enter 定位 · Cmd/Ctrl+Enter 右侧打开笔记 · Esc 返回'});
