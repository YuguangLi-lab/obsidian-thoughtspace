...
export function boardSearchIndex(board:Board,metadata:(n:Card)=>SearchMetadata=()=>({})):BoardSearchEntry[]{
 ...
 return board.nodes.map(n=>{const m=metadata(n),title=n.title||m.title||(n.text&&firstTextLine(n.text))||n.file?.split('/').pop()||`未命名${searchKinds[n.kind]}`,path=n.file||'',location=groups.filter(g=>contained(g,n)).map(g=>({id:g.id,title:g.title||'未命名分组'})),body=n.kind==='text'?n.text||'':m.body||m.headings?.join('\n')||'';
...
function excerptPrefix(body:string,limit:number){
 const nonspace=/\S/g;let match=nonspace.exec(body);if(!match)return'';
 let from=match.index,result='';const size=Math.max(256,limit+1);
 while(from<body.length){
  const end=Math.min(body.length,from+size);result+=body.slice(from,end).replace(/\s+/g,' ');const visible=result.trimEnd();
  if(visible.length>limit)return visible.slice(0,limit)+'…';if(end===body.length)return visible;
  from=end;
  // A long whitespace tail is skipped in one scan, retaining one pending space.
  if(result.endsWith(' ')){nonspace.lastIndex=from;match=nonspace.exec(body);if(!match)return visible;from=match.index;}
 }
 return result.trimEnd();
}
export function searchExcerpt(entry:BoardSearchEntry,query:string,limit=140){
 const trimmed=query.trim();if(!trimmed&&Number.isSafeInteger(limit)&&limit>=0)return excerptPrefix(entry.body,limit);
 const body=entry.body.replace(/\s+/g,' ').trim(),lower=body.toLocaleLowerCase(),words=trimmed.toLocaleLowerCase().split(/\s+/).filter(Boolean),positions=words.map(w=>lower.indexOf(w)).filter(i=>i>=0),start=Math.max(0,(positions.length?Math.min(...positions):0)-32);
