...
export function discloseBranches(board:Board,roots:ReadonlySet<string>,mode:BranchDisclosure){
 ...
 if(mode==='collapse'){for(const id of queue)byId.get(id)!.branchFolded=true;}
 else if(mode==='all'){for(let i=0;i<queue.length;i++){const id=queue[i];if(seen.has(id))continue;seen.add(id);delete byId.get(id)!.branchFolded;for(const child of children.get(id)||[])queue.push(child);}}
  ...
  else for(const child of children.get(id)||[])queue.push(child);
 }}
 board.version=3;
}
