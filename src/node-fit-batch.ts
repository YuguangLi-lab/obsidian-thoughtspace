import {Card,nodeMinimumHeight} from './model';
...
export function nodeFitChanges(nodes:readonly Card[],fits:ReadonlyMap<string,NodeFit>,keys:ReadonlyMap<string,string>,editing:ReadonlySet<string>){
 ...
 for(const n of nodes){const s=fits.get(n.id);
  ...
  if(n.kind!=='image'&&n.kind!=='pdf'&&(n.kind==='text'?!textFitsContent(n):n.kind!=='card'||!n.autoFit))continue;
  if(!Number.isFinite(s.width)||!Number.isFinite(s.height)||s.width<80||s.height<nodeMinimumHeight(n.kind))continue;
  if(Math.abs(s.width-n.width)<1&&Math.abs(s.height-n.height)<1)continue;
  if(n.kind==='text'&&n.textAutoHeight===true&&s.width!==n.width)continue;
