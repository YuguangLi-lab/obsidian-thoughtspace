...
export class InlineCardFit {
 ...
 private async measure(value:string,revision:number){
  ...
  // One-shot scopes safely release registrations arriving after cancellation,
  // so obsolete drafts need not hold native resources until their deadline.
  const release=()=>{if(released)return;released=true;cancel();releaseScope();releaseEditorResource('measurement surface',()=>holder?.remove());};this.release=release;
  try{
   ...
   const render=MarkdownRenderer.render(this.app,body,probe,this.path,scope);
   await Promise.race([render,closed,budget]);
   if(this.disposed||revision!==this.revision)return;
   if(probe.querySelector('.math')||probe.querySelector('mjx-container'))await Promise.race([finishRenderMath(),closed,budget]);
