...
export type ThoughtSpacePreferences=FilingSettings&AppearanceSettings&BoardPreferences&{
 imageHostEnabled?:boolean;database:DatabasePreferences;hub:HubPreferences;favoriteBoards:string[];
...
export function cleanPluginSettings(raw:unknown):ThoughtSpacePreferences {
 ...
 const out:ThoughtSpacePreferences={...defaultFilingSettings,...defaultAppearance,...cleanBoardPreferences(saved),
  database:cleanDatabasePreferences(saved.database),hub:cleanHubPreferences(saved.hub),favoriteBoards:cleanFavorites(saved.favoriteBoards)};
 ...
 if(isOneOf(saved.canvasBackground,['dots','grid','plain']))out.canvasBackground=saved.canvasBackground;
