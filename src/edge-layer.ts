...
type Row={group:SVGGElement;path:SVGPathElement;hit:SVGPathElement;label?:SVGGElement;labelKey?:string;handles?:SVGGElement;geometry?:string;style?:string;route?:ReturnType<typeof connectionPath>;selected?:boolean;unrelated?:boolean;labelSource?:string;handleZoom?:number};
...
export class EdgeLayer{
 ...
 render(board:Board,width:number,height:number,selected?:string,focus?:ReadonlySet<string>){
  ...
  for(const edge of board.edges){const a=nodes.get(edge.from),b=nodes.get(edge.to);if(!a||!b||!intersects(edgeBounds(a,b),view))continue;live.add(edge.id);
   ...
   const style=[edge.color,edge.direction,edge.dashed].join('|'),isSelected=selected===edge.id,isUnrelated=!!focus&&(!focus.has(edge.from)||!focus.has(edge.to));
   // Keep unchanged visible rows alive without touching SVG. Compare values: callers
   // may mutate edges and focus sets in place. Zoom only affects selected handles.
   if(row.geometry===geometryKey&&row.style===style&&row.labelSource===edge.label&&row.selected===isSelected&&row.unrelated===isUnrelated&&(!isSelected||row.handleZoom===board.viewport.zoom))continue;
   ...
   if(selected===edge.id){if(!row.handles){row.handles=svg(row.group,'g','ts-edge-handles');for(const end of ['from','to'] as const){const h=svg(row.handles,'circle','ts-edge-handle');h.dataset.edgeEnd=end;h.dataset.edge=edge.id;attr(h,'role','button');attr(h,'aria-label',end==='from'?'拖动重接起点':'拖动重接终点');}}for(const h of Array.from(row.handles.children)){const pt=geometry[(h as SVGElement).dataset.edgeEnd as 'from'|'to'];attr(h,'cx',pt.x);attr(h,'cy',pt.y);attr(h,'r',6/board.viewport.zoom);}}
   else {row.handles?.remove();row.handles=undefined;}
   row.labelSource=edge.label;row.selected=isSelected;row.unrelated=isUnrelated;row.handleZoom=board.viewport.zoom;
  }
