...
type Row={group:SVGGElement;path:SVGPathElement;hit:SVGPathElement;label?:Caption;labelX?:number;labelY?:number;handles?:SVGGElement;handleEnds?:SVGCircleElement[];geometry?:Geometry;style?:Style;route?:ReturnType<typeof connectionPath>;selected?:boolean;singleSelected?:boolean;unrelated?:boolean;labelSource?:string;handleZoom?:number;frame?:symbol;order?:number};
...
export class EdgeLayer{
 ...
 render(board:Board,width:number,height:number,selected?:string,focus?:ReadonlySet<string>,batchSelected?:ReadonlySet<string>){
  ...
  for(const edge of board.edges){const index=order++,a=nodes.get(edge.from),b=nodes.get(edge.to);if(!a||!b||!edgeInView(a,b,view))continue;
   ...
   if(restyle){
    // Endpoint motion keeps the same paint. Marker names and definitions only
    // need resolving when a visible row is created or its style changes.
   ...
   // Labels may contain a long source title. Keep geometry separate instead of
   // embedding the whole label in a fresh comparison string on every drag frame.
   if(row.labelSource!==edge.label||row.labelX!==geometry.label.x||row.labelY!==geometry.label.y){
    if(edge.label){
     ...
     if(caption.source!==edge.label){let label=edge.label;if(label.length>80){let end=79;if(/[\uD800-\uDBFF]/.test(label[end-1])&&/[\uDC00-\uDFFF]/.test(label[end]))end--;label=label.slice(0,end)+'…';}caption.width=Math.min(900,Array.from(label).reduce((n,c)=>n+(/[\u3000-\uffff]/.test(c)?12:6.5),0)+18);attr(caption.bg,'width',caption.width);if(caption.text.textContent!==label)caption.text.textContent=label;caption.title.textContent=edge.label;caption.source=edge.label;}
    ...
    row.labelX=geometry.label.x;row.labelY=geometry.label.y;
   }
   if(isSingleSelected){if(!row.handles){row.handles=svg(row.group,'g','ts-edge-handles');row.handleEnds=[];for(const end of ['from','to'] as const){const h=svg(row.handles,'circle','ts-edge-handle');row.handleEnds.push(h);h.dataset.edgeEnd=end;h.dataset.edge=edge.id;attr(h,'role','button');const title=svg(h,'title',''),titleId=`${this.prefix}-handle-title-${++handleTitle}`;attr(title,'id',titleId);title.textContent=end==='from'?'拖动重接起点':'拖动重接终点';attr(h,'aria-labelledby',titleId);}}for(const h of row.handleEnds!){const pt=geometry[h.dataset.edgeEnd as 'from'|'to'];attr(h,'cx',pt.x);attr(h,'cy',pt.y);attr(h,'r',6/board.viewport.zoom);}}
   else {row.handles?.remove();row.handles=undefined;row.handleEnds=undefined;}
  ...
  for(const[id,row]of this.rows)if(row.frame!==frame){row.group.remove();this.rows.delete(id);}
  // Re-entering rows and in-place edge reorders must preserve the board's paint
  // and hit-test order. Ordinary camera/geometry frames never touch DOM order.
  if(reorder){let next=this.defs.nextSibling;for(const edge of board.edges){const row=this.rows.get(edge.id);if(!row)continue;if(row.group!==next)this.root.insertBefore(row.group,next);next=row.group.nextSibling;}}
 }
}
