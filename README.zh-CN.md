...
- 拖入已有笔记：从 Obsidian 文件列表、搜索结果或内部链接拖到白板，在落点生成关联卡片；支持多选批量排列和整批撤销，原文件保持原位。
- 原生文件联动：在 Obsidian 中改名或移动笔记、PDF、目录后，生成的摘录来源及上次写作草稿路径同步更新；无法确定的同名来源保留原样，普通正文和代码示例不改写。
- 白板写作：组合卡片与分组，生成 Markdown 草稿，保留参考材料。
- Obsidian 联动：文件、标签、属性、链接和编辑工具栏；日历与日记已拆为独立插件，可选安装。

## 白板编辑与操作

选择卡片或文本后，顶部悬浮栏可直接切换 **编辑、文字、背景、边框**。切换样式保留正在编辑的草稿与选区；窄分屏使用图标入口，悬停可查看名称。卡片上方提供预览与折叠，右下角手柄调整大小；右侧子节点控件用于分支折叠与逐层展开。

左侧 **插入内容** 将已有笔记、图片、PDF、表格与结构操作分区。**更多白板工具** 支持搜索、最近使用和分类定位。

## 阅读桌与引用预览

从白板顶部 **工作区 → 阅读桌** 进入。左侧搜索与筛选材料，并在内容、目录和关联之间切换；右侧专注阅读，顶部可标记阅读状态、打开原笔记或定位回白板。底部翻篇保持可见，也可用 `Alt + ← / →` 切换材料。修改阅读状态不会重置正文滚动。

编辑笔记时，选择文字并打开关联笔记工具，选择笔记中的标题或块，再点“阅读引用原文”可预览范围。底部直接插入链接或嵌入正文，继续使用 Obsidian 原生双链与撤销。
