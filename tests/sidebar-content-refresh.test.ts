...
import {outlineNodes,taskSummary,visibleTasks} from '../src/navigation';
...
class Element {
 children:Element[]=[];style={paddingLeft:'',setProperty(){}};dataset:Record<string,string>={};
 ...
 toggleClass(){}setAttribute(){}addEventListener(){}
...
function fixture(tab:Tab){
 ...
 const deps={window,TFile:File,EXT:'thoughtspace',isWorkspaceFile,libraryFiles,noteExcerpt,firstNoteReferences,outlineNodes,taskSummary,visibleTasks,extractTasks,
  act:(run:()=>unknown)=>{pending.push(Promise.resolve().then(run));},button:(host:Element)=>host.createEl(),setIcon(){},getAllTags:()=>[],readingTitle:()=>''};
 ...
 Object.assign(view,{sidebarRun:0,renderFrame:0,viewportOnlyRender:true,tab,boardScope:'spaces',boardSort:'title',query:'',tag:'',outlineKind:'all',libraryScope:'vault',librarySort:'updated',taskScope:'board',taskFilter:'all',selected:new Set(),collapsedBoards:new Set(),file:files[0],session:{board},plugin,
  app:{vault,metadataCache:{on:(event:string,fn:(file:File)=>void)=>on('metadata',event,fn),getFileCache:()=>({})}},registerEvent(){},matches:()=>true,
  renderBoard(){calls.canvas++;},renderSaveStatus(){calls.status++;},populateSidebar(){calls.build++;return view.buildSidebar(new Element(),view.sidebarRun);}});
 view.init();
 ...
 return {view,plugin,board,calls,files,note,unrelated,pdf,emit,advance,timers,frames};
}
