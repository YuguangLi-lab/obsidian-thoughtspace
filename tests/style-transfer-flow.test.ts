...
const View=new Function('readNodeStyle','applyNodeStyle','fitTextNode','Notice','textFitsContent',transformSync(`class View{${methods}};return View`,{loader:'ts'}).code)(readNodeStyle,applyNodeStyle,()=>{throw Error('Fixed object styles must not resize text');},class{},textFitsContent);
...
test('pasting appearance into ordinary fixed text preserves its manual geometry',()=>{
 for(const textAutoHeight of [undefined,false]){const {v,owner,history}=fixture(),target=owner.board.nodes[1];Object.assign(target,{kind:'text',text:'Long fixed text',autoSize:true,textAutoHeight});delete target.file;const before={...target};v.copyObjectStyle(new Set(['a']),owner);v.pasteObjectStyle(new Set(['b']),owner);assert.equal(history.length,1);assert.equal(target.color,'blue');assert.equal(target.fontSize,24);assert.deepEqual([target.width,target.height,target.text,target.textAutoHeight],[before.width,before.height,before.text,before.textAutoHeight]);}
});
